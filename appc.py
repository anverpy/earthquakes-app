...
# Custom CSS to change sliders and UI elements from red to #00BFFF (Deep Sky Blue)
st.markdown("""
<style>
...
st.title("🌍 Interactive Dashboard of Seismic Activity")
st.markdown("""
...
def load_data():
    try:
        ...
        df['magnitude_category'] = np.select(conditions, choices, default='Unclassified')
    ...
    if df is not None and not df.empty:
        ...
        if len(filtered_df) == 0:
            ...
            with main_tabs[0]:
                ...
                top_places.columns = ['Region', 'Event Count']
            ...
            with main_tabs[1]:
                ...
                with geo_tabs[1]:
                    ...
                    st.markdown("""
                    This heat map shows areas with the highest concentration of seismic activity.
                    Brighter zones indicate higher density of events.
                    """)
                ...
                with geo_tabs[2]:
                    ...
                    It uses the DBSCAN algorithm to group events based on their spatial proximity.
                    """)
                    ...
                    if len(filtered_df) > 10:  # Ensure there is enough data
                        ...
                        col2.metric("Unclustered events (noise)", n_noise)
                                ...
                                if cluster_options:
                                    ...
                                    if not cluster_data.empty:
                                        ...
                                        **Cluster Interpretation:**
                                        ...
                                        Observe the temporal evolution to identify whether it is simultaneous or sequential events.
                                        """)
                    else:
                        st.warning("Not enough data to perform clustering analysis with the current filters.")
            
            # Tab 3: Temporal Analysis
            with main_tabs[2]:
                st.subheader("Analysis of Temporal Patterns")
                ...
                with temp_tab2:
                    try:
                        ...
                        # Order days
                        dow_data['Ordered_Day'] = pd.Categorical(dow_data['Day'], categories=ordered_days, ordered=True)
                        dow_data = dow_data.sort_values('Ordered_Day')
                        ...
                        fig_dow = px.bar(
                            ...
                            text='Count',
                            title='Event Distribution by Day of the Week',
                            color_continuous_scale='Viridis',
                        ...
                        - Trends that deserve further investigation
                        - Note that in natural phenomena, weekly patterns are generally not expected
                        """)
                        
                        # Create a heatmap of activity by weekday and week of the month
                        st.subheader("Heat Map: Activity by Week and Day")
                        
                        # Add column for relative week number within the period
                ...
                with temp_tab3:
                    try:
                        st.subheader("Event Distribution by Hour of the Day")
                        ...
                        fig_hourly = px.bar(
                            ...
                            y='Count',
                            color='Average Magnitude',
                            title="Distribution of seismic events by hour of the day",
                            labels={"Hour": "Hour of the day (UTC)", "Count": "Number of events"},
                            color_continuous_scale='Viridis',
                            text='Count'
                        )
                        ...
                        fig_heatmap = px.imshow(
                            pivot_data,
                            labels=dict(x="Hour of the Day (UTC)", y="Day of the Week", color="Number of Events"),
                ...
                with adv_tab3:
                    try:
                        ...
                        if len(numeric_cols) >= 2:
                            ...
                            # Flatten columns
                            cat_stats.columns = [
                                'Category', 'Count', 'Average Magnitude', 'Magnitude Std Dev', 
        ...
        st.error("Seismic data could not be loaded. Verify that the file 'all_month.csv' exists and is correctly formatted.")

except Exception as e:
    st.error(f"Error loading or processing data: {e}")
    st.info("Verify that the file 'all_month.csv' is available and correctly formatted.")
